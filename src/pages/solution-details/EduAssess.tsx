...
import { GraduationCap, Mic, PenLine, BookOpen, Zap, MessageSquare, TrendingUp, ChevronDown, CheckCircle, ExternalLink } from 'lucide-react';
import { useTranslation } from 'react-i18next';

const FEATURE_ICONS = [Mic, PenLine, BookOpen, Zap, MessageSquare, TrendingUp];

const EduAssess = () => {
  const { t } = useTranslation('eduassess');
    ...
    <div>
      ...
      <section className="hook-section min-h-screen flex items-center relative" data-theme="dark">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-20 right-[10%] w-72 h-72 bg-cyan-500/15 rounded-full blur-3xl animate-float" />
          <div className="absolute bottom-20 left-[5%] w-96 h-96 bg-teal-400/10 rounded-full blur-3xl animate-float-delayed" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <Button
                  className="bg-white text-purple-900 hover:bg-white/90 font-semibold rounded-full px-8 py-5 sm:py-6 text-base shadow-xl hover:-translate-y-0.5 transition-all duration-300"
                  asChild
                >
                  <a href="https://www.eduassess.space/" target="_blank" rel="noopener noreferrer">
                    {t('hero.try_now')}
                ...
                <Button
                  variant="outline"
                  className="bg-white/10 text-white border-white/20 hover:bg-white/20 font-semibold rounded-full px-8 py-5 sm:py-6 text-base transition-all duration-300 hover:-translate-y-0.5"
                  asChild
                >
            ...
            <div className="relative">
              <div className="absolute -inset-1 bg-gradient-to-r from-cyan-500/40 to-teal-600/40 rounded-2xl blur-lg" />
              <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-white/10">
                <img
                  src="/images/eduassess-hero.png"
                  alt="EduAssess AI — English Language Assessment Platform"
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
              {steps.map((s) => (
                <div key={s.step} className="bg-white/10 border border-white/10 rounded-2xl p-6 sm:p-8 text-center hover:bg-white/15 transition-colors">
                  <div className="text-4xl font-extrabold text-cyan-400/60 mb-4">{s.step}</div>
      ...
      <CTASection
        ...
        primaryButtonLink="https://www.eduassess.space/"
